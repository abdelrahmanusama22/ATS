@php
    ...
    $lang = $isArabic ? [
        ...
        'email' => 'sales@ats.com',
        'langName' => 'English',
        'langLink' => '/en', // Placeholder for locale switch
        'myAccount' => 'حسابي',
        'myOrders' => 'طلباتي',
        'cart' => 'السلة',
        'items' => 'عناصر',
        ...
        'email' => 'sales@ats.com',
        'langName' => 'العربية',
        'langLink' => '/ar', // Placeholder for locale switch
    ...
    <div class="bg-background border-b border-gray-border text-[12px] text-gray-body h-[40px] hidden md:flex items-center">
        <div class="max-w-[1280px] w-full mx-auto px-6 md:px-10 flex justify-between">
            ...
            <div class="flex items-center gap-6">
                ...
                <a href="{{ route('account') }}" class="cursor-pointer hover:text-dark">{{ $lang['myAccount'] }}</a>
                <a href="{{ route('orders') }}" class="cursor-pointer hover:text-dark">{{ $lang['myOrders'] }}</a>
                    ...
                    <div class="absolute top-[100%] {{ $isRtl ? 'left-0' : 'right-0' }} w-[320px] bg-white border border-gray-border rounded-[12px] shadow-lg opacity-0 invisible group-hover/cart:opacity-100 group-hover/cart:visible transition-all duration-200 z-50 overflow-hidden translate-y-2 group-hover/cart:translate-y-0 text-dark">
                        ...
                        <div class="p-4 border-t border-gray-border bg-gray-50/50">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('cart') }}" class="flex-1 h-[38px] flex items-center justify-center bg-white border border-gray-border rounded-md text-[13px] font-medium hover:bg-gray-50 transition-colors">{{ $lang['viewCart'] }}</a>
                                <a href="{{ route('checkout') }}" class="flex-1 h-[38px] flex items-center justify-center bg-primary text-white rounded-md text-[13px] font-medium hover:bg-[#C4182A] transition-colors shadow-sm">{{ $lang['checkout'] }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="max-w-[1280px] w-full mx-auto px-6 md:px-10 flex justify-between items-center text-[14px]">
            <div class="flex items-center gap-6">
                ...
                <div class="hidden lg:flex items-center gap-8 font-medium">
                    <a href="{{ route('home') }}" class="{{ activeClass(['/', 'index.html'], $currentPath) }} transition-colors">{{ $lang['home'] }}</a>
                    <a href="{{ route('catalog') }}" class="{{ activeClass(['catalog', 'product'], $currentPath) }} transition-colors">{{ $lang['products'] }}</a>
                    <a href="{{ route('solutions') }}" class="{{ activeClass(['solutions'], $currentPath) }} transition-colors">{{ $lang['solutions'] }}</a>
                    <a href="{{ route('services') }}" class="{{ activeClass(['services'], $currentPath) }} transition-colors">{{ $lang['services'] }}</a>
                    <a href="{{ route('about') }}" class="{{ activeClass(['about'], $currentPath) }} transition-colors">{{ $lang['about'] }}</a>
                </div>
            </div>
