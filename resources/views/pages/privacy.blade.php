...
@section('content')
    <main class="max-w-[980px] mx-auto px-6 py-14 md:py-20">
        @if($page)
        <header class="mb-10">
            <span class="inline-block bg-red-muted text-primary text-[12px] font-bold px-3 py-1 rounded-full mb-4">{{ __('Policy') }}</span>
            <h1 class="text-[32px] md:text-[44px] font-bold leading-tight mb-3">{{ $page->title }}</h1>
            <p id="privacy-updated-date" class="text-[14px] text-gray-body">{{ __('Last updated:') }} {{ $page->updated_at->format('F j, Y') }}</p>
        </header>

        <section id="privacy-content" class="space-y-8 text-[15px] leading-7 text-gray-body prose prose-lg max-w-none">
            {!! $page->content !!}
        </section>
        @else
        <header class="mb-10">
            <span class="inline-block bg-red-muted text-primary text-[12px] font-bold px-3 py-1 rounded-full mb-4">{{ __('Policy') }}</span>
            <h1 class="text-[32px] md:text-[44px] font-bold leading-tight mb-3">{{ __('Privacy Policy') }}</h1>
        </header>
        <section id="privacy-content" class="space-y-8 text-[15px] leading-7 text-gray-body">
            <p>{{ __('Privacy policy content is currently being updated.') }}</p>
        </section>
        @endif
