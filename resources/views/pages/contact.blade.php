...
@section('content')
    <main class="max-w-[1280px] mx-auto px-6 md:px-10 py-16 md:py-24">
        ...
        <div class="text-center mb-16">
            <h1 class="text-[40px] md:text-[56px] font-space font-bold text-dark leading-tight tracking-tight mb-4">{{ $contentBlocks['contact_hero_title']->content ?? 'Get in Touch' }}</h1>
            <p class="text-[18px] text-gray-body max-w-[650px] mx-auto leading-relaxed">{{ $contentBlocks['contact_hero_desc']->content ?? 'Whether you need a hardware quote, technical support, or a complete infrastructure consultation, our team is ready to assist.' }}</p>
        </div>
        ...
        <div class="grid lg:grid-cols-[1fr_1.5fr] gap-12 lg:gap-20 items-start">
            ...
            <div class="space-y-10">
                <div class="flex gap-4">
                    ...
                    <div>
                        <h3 class="font-bold text-[18px] text-dark mb-2">{{ __('Sales & General Inquiries') }}</h3>
                        <p class="text-[15px] text-gray-body mb-1">{{ __('Mon-Thu: 9:00 AM - 5:00 PM') }}</p>
                        <a href="tel:{{ str_replace(' ', '', $globalSettings['contact_phone'] ?? '+201234567890') }}" class="text-[16px] font-bold text-primary hover:underline block">{{ $globalSettings['contact_phone'] ?? '+20 123 456 7890' }}</a>
                    </div>
                </div>
                ...
                <div class="flex gap-4">
                    ...
                    <div>
                        <h3 class="font-bold text-[18px] text-dark mb-2">{{ __('Email Support') }}</h3>
                        <p class="text-[15px] text-gray-body mb-1">{{ __('Guaranteed 2-hour response during business hours.') }}</p>
                        <a href="mailto:{{ $globalSettings['contact_email'] ?? 'support@ats-egypt.com' }}" class="text-[16px] font-bold text-primary hover:underline block">{{ $globalSettings['contact_email'] ?? 'support@ats-egypt.com' }}</a>
                    </div>
                </div>
                ...
                <div class="flex gap-4">
                    ...
                    <div>
                        <h3 class="font-bold text-[18px] text-dark mb-2">{{ __('Headquarters') }}</h3>
                        <p class="text-[15px] text-gray-body leading-[1.6]">{{ $globalSettings['contact_address'] ?? '123 Tech Avenue, Smouha' }}</p>
                    </div>
                </div>

                <!-- Google Maps Embed placeholder -->
                <div class="w-full h-[200px] bg-gray-200 rounded-xl overflow-hidden border border-gray-border mt-8 relative">
                    <img src="{{ isset($contentBlocks['contact_map_image']) && $contentBlocks['contact_map_image']->hasMedia('default') ? $contentBlocks['contact_map_image']->getFirstMediaUrl('default') : asset('assets/images/downloaded/photo_1524661135_423995f22d0b.jpg') }}" alt="Map" class="w-full h-full object-cover opacity-80 mix-blend-multiply">
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-border rounded-2xl p-8 lg:p-12 box-shadow">
                ...
                <form action="{{ route('contact.submit') }}" method="POST" id="contactForm" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="subject" class="input-field appearance-none bg-white font-medium" required>
                            ...
                            <option value="quote" {{ old('subject') == 'quote' ? 'selected' : '' }}>{{ __('Hardware Quote Request') }}</option>
